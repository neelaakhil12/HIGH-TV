...
export default function PhotosPage({ articles = [] }: { articles?: Article[] }) {
  ...
  return (
    <div className="select-none">
      {/* Grid of Photos */}
      {items.length === 0 ? (
        <div className="text-center py-16 text-gray-400 font-bold telugu-text border border-dashed border-gray-250 rounded-none">
          గ్యాలరీలో ఫోటోలు ఏవీ లేవు.
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {items.map((img, index) => (
            <div
              key={img.id}
              className="flex flex-col group cursor-pointer bg-white border border-gray-150 rounded-none overflow-hidden shadow-xs hover:shadow-md transition-all duration-300"
